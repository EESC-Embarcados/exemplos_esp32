...
GPIOViewer gpio_viewer; // global vaiable for the GPIO Viewer
...
void setup() 
{
  ...
  pinMode(4, OUTPUT);  // pino correspondente ao LED onboard como saída
  digitalWrite(4, LOW); //  inicialmente desligado (LED apagado)
 ...
 // na função Serial.print não ocorre quebra de linha
  ...
  gpio_viewer.connectToWifi("iPhone", "");
  gpio_viewer.setPort(5556);                 // You can set the http port, if not set default port is 8080
...
void loop() 
{
 ...
 // Serial.println(capacitiveValue); // e o valor lido no monitor serial
  ...
  if (touchRead(13) < threshold) {  // lógica para acender o LED onboard quando o pino13 for tocado
    digitalWrite(4, HIGH); // se sim, acende o LED (pino 2)...
    //Serial.println("touched"); // msg p/ sinalizar quando houve toque
    } 
    else {
      digitalWrite(4, LOW); // caso contrário, o LED permanece apagado
      }
